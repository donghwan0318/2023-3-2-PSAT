...
Data_date <- X_r1$time
# 직접적으로 영향을 안 미칠 것 같은 변수 제거 (생략 가능)
X_r1 %<>%
  select(-c(humidity, ground_press, wind_dir, month, day, hour, time, year))
...
####### Univariate #######
y <- zoo(as.numeric(train_Y), train_X$time)
